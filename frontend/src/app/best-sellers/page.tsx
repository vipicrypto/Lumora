    ...
    async function load() {
      try {
        const response = await fetch(
          "/api/products?section=best&limit=100",
          { cache: "no-store" }
        );
        ...
        <div className="max-w-7xl mx-auto px-5 sm:px-6 lg:px-8 py-14 md:py-20">
          <div className="max-w-3xl">
            ...
            <p className="mt-5 text-base md:text-lg text-[#6f6963] leading-7 max-w-2xl">
              The gifts people keep coming back for — thoughtfully chosen
              favorites that have earned their place in the Lumora collection.
            </p>
          </div>
        </div>
      </section>
        ...
        {loading ? (
          ...
          <>
            <div className="flex items-center justify-between mb-7">
              <p className="text-sm text-[#77716b]">
                {products.length}{" "}
                {products.length === 1 ? "product" : "products"} to discover
              </p>
            </div>
          ...
          <div className="py-20 text-center">
            ...
            <p className="mt-3 text-[#77716b]">
              Our most-loved products will appear here soon.
            </p>

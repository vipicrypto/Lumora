    ...
    async function load() {
      try {
        const response = await fetch(
          "/api/products?section=featured&limit=4",
          { cache: "no-store" }
        );
  ...
  return (
    <section>
      <div className="flex items-end justify-between mb-7">
        ...
        <Link href="/featured-gifts" className="hidden sm:flex items-center gap-1 text-sm font-medium text-[#8b6f5a] hover:text-[#2d2a26] transition-colors">
          View all
